    ...
    for (const p of mockProposals) {
      const result = await db.run(
        `INSERT INTO proposals (title, desc, bairro, tipo, location, tags, status, author_id, author_name, author_initials, ia, emoji, created_at, photo)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`,
        [p.title, p.desc, p.bairro, 'calcada', p.location, JSON.stringify(p.tags), p.status, 1, p.author_name, p.author_initials, JSON.stringify(p.ia), p.emoji, p.created_at]
      );
